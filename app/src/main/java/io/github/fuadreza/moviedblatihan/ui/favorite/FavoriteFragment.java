...
//        recycler.setAdapter(adapter);
        ...
        adapter = new FavoriteAdapter(getContext(), FavoriteFragment.this);
        recycler.setAdapter(adapter);

        if (savedInstanceState == null) {
            new LoadMovieAsync(getContext(), FavoriteFragment.this).execute();
        } else {
...
//        recycler.setAdapter(adapter);
    ...
    public void onClick(Movie movie) {
        Intent intent = new Intent(getActivity(), DetailMovieActivity.class);
        Uri uri = Uri.parse(FavoriteContentProvider.CONTENT_URI + "/" + movie.getId());
        intent.putExtra(DetailMovieActivity.MOVIE_ID, movie.getId());
        intent.setData(uri);
        startActivity(intent);
    }
